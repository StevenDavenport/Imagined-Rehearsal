# RLScape Stage 1B: Exact Imagination-Path Audit

**Status:** Complete. At the audited six-step horizon, 94--97% of the actor-IR
lambda-return target came from critic bootstrap.

Stage 1A established that factual reward and continuation prediction remain
...
The workflow uses the same immutable Stage 1A selection: 64 successful and 64
...
This is up to 7,680 anchor--goal banks, 983,040 MCPB posterior samples, and about
...
Raw per-posterior-sample, per-timestep shards retain:
...
The historical `--particles` flag and `*_particle_rate` result columns are
retained solely as stable schema names for completed artifacts. In scientific
prose they denote MCPB posterior samples; no particle filter, weighting, or
resampling is used.
...
The completed Stage 1A audit must remain at its historical artifact path,
`<experiment-root>/head_audit`.
