...
4e-5, and 128 posterior particles.
...
5. Reward-only actor IR at each horizon. This uses predicted rewards and
   continuation but sets the value bootstrap to zero. Zero-reward particles
   contribute no policy-gradient signal. Entropy is also zero so it cannot
   dominate the sparse reward signal.
