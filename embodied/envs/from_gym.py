...
class FromGym(embodied.Env):
  ...
  def step(self, action):
    if action['reset'] or self._done:
      self._done = False
      obs, self._info = self._reset()
      return self._obs(obs, 0.0, is_first=True)
    if self._act_dict:
      action = self._unflatten(action)
    else:
      action = action[self._act_key]
    obs, reward, self._done, is_terminal, self._info = self._step(action)
    return self._obs(
        obs, reward,
  ...
  def _step(self, action):
    ...
    if isinstance(output, tuple) and len(output) == 5:
      ...
      return obs, reward, done, is_terminal, info
    if isinstance(output, tuple) and len(output) == 4:
      obs, reward, done, info = output
      info = info or {}
      is_terminal = bool(info.get('is_terminal', done))
      return obs, reward, bool(done), is_terminal, info
    raise RuntimeError(f'Unsupported gym step output: {type(output)} len={len(output)}')
