...
\date{Experiments completed August 2026; expanded report generated 15 August 2026}

\begin{document}
\maketitle

\begin{abstract}
This chapter records five linked studies in a goal-conditioned DreamerV3 agent operating in RLScape. Study I compares a 200k-capacity global FIFO replay against a persistent, goal-stratified episode reservoir while one agent learns \emph{kill goblin}, \emph{bury bones}, and \emph{chop logs} for 500k environment steps each. Reservoir replay produced strong but transient balanced retention: at 1.25M steps its worst sampled-policy goal reached \pct{49} versus \pct{8} for FIFO, but by 1.5M all reservoir goals had collapsed to at most \pct{16}. Study II finds accurate factual completion recognition, weak one-hot goal semantics, a severely optimistic critic, and a six-step IR target that is \pct{94.4}--\pct{97.3} critic bootstrap. Study III causally isolates the actor: critic-free reward-only IR recovers a corrupted policy, reaching \pct{73.0} sampled success at $H=15$ and \pct{85.3} deterministic success at $H=20$. Study IV transfers this mechanism to natural continual-learning damage. At the collapsed 1.5M checkpoint, reward-only $H=15$ recovers bury bones from \pct{14} to \pct{91} sampled and from \pct{4} to \pct{86} deterministic, while standard IR remains unsafe. Study V locates the critic failure. On the same 48,299 real posterior states, mean value bias grows from $+1.184$ at 1.25M to $+1.595$ at 1.5M; the slow critic copies this error, imagined targets plateau after $H=3$, and actor-selected rollouts further prefer high-bootstrap, often lower-reward trajectories. Thus actor/model exploitation amplifies rather than originates an already corrupted value landscape. The combined evidence establishes that imagined actor adaptation is viable on synthetic and natural policy deficits, but the current Dreamer critic is not a safe teacher. Bounded value learning, explicit counterfactual goal supervision, and prespecified adaptation gates are the next requirements for a continual-learning algorithm rather than an episode-local recovery mechanism.
\end{abstract}
...
\draw[ar] (s5)--(s6);
\end{tikzpicture}
\caption{Chronological experimental logic. Each study was designed after observing the previous result; Studies II--V reuse immutable checkpoints and therefore isolate mechanisms without another continual-training run.}
...
The initial hypothesis---that model exploitation slowly inflates critic targets during continual learning---is therefore only partly correct. Actor exploitation is measured directly, but the larger error precedes it: the critic is already badly calibrated at real posterior states. Nor does value grow progressively from $H=6$ to $H=20$. The remaining upstream question concerns critic learning itself: recursive target bias, goal-semantic aliasing, continual distribution shift, or their interaction. Longer continual runs may amplify these mechanisms, so bounded value targets and explicit counterfactual supervision are likely to become more important rather than less.
...
\begin{enumerate}
  \item \textbf{Stage 4A: counterfactual and bounded head repair.} Before another continual run, freeze the retained representation and actor at a pinned checkpoint and retrain diagnostic reward/value heads from preserved experience. Generate semantically valid targets for every goal, constrain success-return values to their attainable range, and evaluate on held-out real states. Repeat the Study IIa and Study V audits before allowing the repaired value into IR.
  \item \textbf{Stage 4B: conservative critic reintroduction.} Compare the established reward-only teacher against a calibrated bounded bootstrap, clipped advantage, uncertainty penalty, short trusted bootstrap, or $J=J_{\mathrm{RO}}+\beta J_{\mathrm{value}}$ with small or state-gated $\beta$. Stage IV shows that the critic is occasionally useful, so complete removal need not be the final algorithm; Stage V shows that the present critic must not be reused unchanged.
  \item \textbf{Stage 5: prespecified IR gate and trust region.} Use Stage IV only to nominate signals; freeze the decision rule before evaluation. The gate must detect when adaptation is needed without privileged historical success, while an action-KL or parameter-update constraint limits harm. Evaluation should report recovered weak cells, damaged competent cells, and compute avoided.
...
\begin{itemize}
  ...
  \item \textbf{No persistent learning in Studies III--IV.} Adaptation resets each episode. The studies establish test-time recovery, not yet a continual-learning update rule that safely carries knowledge forward.
...
Study V explains the failed teacher more precisely. On identical real states, critic bias grows from $+1.184$ to $+1.595$ as ranking collapses; the slow critic mirrors the error; target scale plateaus by short horizons; and the actor selects additional high-bootstrap, often lower-reward trajectories. The research direction therefore remains viable, but its center of gravity changes: the immediate opportunity is not more ungated Dreamer actor updates. It is \textbf{critic-free or conservatively critic-gated imagined adaptation, supported by bounded value learning, explicit counterfactual goal training, and replay that preserves old experience without overwhelming current-task learning}.

This is a positive result in the intended scientific sense. It identifies a condition under which IR works strongly on both synthetic and natural policy deficits, identifies and localizes the learned component that makes the default method fail, exposes the task-specific harm that still requires gating, and leaves a checkpointed, reproducible ladder for converting episode-local recovery into a continual reinforcement-learning algorithm. The next constructive study should repair goal semantics and value calibration at a pinned checkpoint before another costly continual-training run.
...
\section{Study II--V reproducibility manifest}
...
The expanded package contains the original compact Study I tables, Stage II CSV/JSON summaries and vector figures, the complete Study III result, comparison, corruption-selection, and specification files, all 60 Study IV result rows with paired effects, and the complete compact Stage V calibration, target-provenance, actor-contrast, specification, integrity, and figure artifacts. The included \texttt{generate\_figures.py} regenerates the report-native plots from these audited compact inputs. Raw episode and transition logs remain on the experiment machine and are not duplicated into the report package.
