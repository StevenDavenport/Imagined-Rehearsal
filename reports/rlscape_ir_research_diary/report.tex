...
\draw[ar] (s6)--(s7);
\end{tikzpicture}
\caption{Chronological experimental logic. Each study was designed after observing the previous result; Studies II--VI reuse immutable checkpoints and therefore isolate mechanisms without another continual-training run.}
...
\begin{enumerate}
  ...
  \item \textbf{Stage 4B: conservative value-dose and stability assay.} From the same pinned checkpoint, fully pair environment and agent PRNG streams and repeat adaptation seeds. Compare reward-only against $J=J_{\mathrm{RO}}+\beta J_{\mathrm{B}}$ over a small prespecified $\beta$ grid, clipped bounded advantages, short bootstrap windows, and update/KL caps. The primary endpoint is Pareto improvement in mean and worst-goal success, not average gain alone.
  \item \textbf{Stage 5: prespecified IR gate and trust region.} Use Studies IV and VI only to nominate signals; freeze the decision rule before evaluation. The gate must detect when adaptation is needed without privileged historical success, while an action-KL or parameter-update constraint limits harm. Evaluation should report recovered weak cells, damaged competent cells, compute avoided, and false-positive gate activations.
...
\begin{itemize}
  ...
  \item \textbf{No persistent learning in Studies III, IV, or VI.} Adaptation resets each episode. The studies establish test-time recovery and mechanism repair, not yet a continual-learning update rule that safely carries knowledge forward.
...
The behavioral result prevents an overly simple conclusion. Factual-only reward IR, not the semantically strongest counterfactual head, gives the best sampled profile; bounded-bootstrap IR is much safer than the original critic but still damages chop. The research direction therefore remains viable, but its center of gravity changes again: the immediate opportunity is not simply ``fix the critic and turn standard IR back on.'' It is \textbf{critic-free adaptation as the reference teacher, conservative and dose-controlled value reintroduction, fully paired stochastic assays, and only then a prespecified gate and trust region supported by replay that preserves old experience without overwhelming current-task learning}.

This is a positive result in the intended scientific sense. It identifies a condition under which IR works strongly on both synthetic and natural policy deficits, identifies and localizes the learned component that makes the default method fail, directly repairs goal semantics and value scale, and shows which failure remains after those repairs. It also exposes task-specific harm and stochastic-assay variance that must be resolved before gating. The next constructive study should use the pinned repaired checkpoint for a conservative value-dose and PRNG-paired stability assay; another costly continual-training run is not yet required.
...
The expanded package contains the original compact Study I tables, Stage II CSV/JSON summaries and vector figures, the complete Study III result, comparison, corruption-selection, and specification files, all 60 Study IV result rows with paired effects, the complete compact Study V calibration and target-provenance artifacts, and the Stage VI immutable specification, repair manifests, five held-out audit summaries, all 48 evaluation rows, paired effects, and integrity records. The included \texttt{generate\_figures.py} regenerates the report-native plots from these audited compact inputs. Raw episode and transition logs remain on the experiment machine and are not duplicated into the report package.
