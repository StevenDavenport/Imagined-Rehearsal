...
checkpoint using the immutable Stage 1A replay selection: 64 successful and 64
...
2. **The weak checkpoint is more optimistic while imagining fewer successes.**
   ...
   the inverse relationship from Stage 1A: critic confidence increases as real
...
3. **Goal identity scarcely changes the imagined target.** Strong-checkpoint
   ...
   counterfactual semantic failure found in Stage 1A.
...
5. **Reward and stopping are detected as visual events, not goal-specific
   ...
   reward head. This is internally consistent with Stage 1A: the heads recognize
